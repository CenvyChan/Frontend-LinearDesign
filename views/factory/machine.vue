...
defineOptions({ name: 'FactoryMachine' });
...
const currentRow = ref<Machine | null>(null);
...
const formData = reactive<Partial<Machine>>({
  code: '',
  name: '',
  model: '',
  status: 'NORMAL',
...
const fetchData = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      ElMessage.error(res.message || '获取机台设备列表失败');
    }
  } catch {
    ElMessage.error('获取机台设备列表失败');
  } finally {
    loading.value = false;
  }
};

const openDialog = (row?: Machine) => {
  if (row) {
    isEdit.value = true;
    currentId.value = row.id;
    dialogTitle.value = '编辑机台设备';
    Object.assign(formData, {
      code: row.code,
      name: row.name,
      model: row.model || '',
      status: row.status || 'NORMAL',
      workCenterId: row.workCenterId || 0,
    ...
    dialogTitle.value = '新建机台设备';
    formData.code = '';
    formData.name = '';
    formData.model = '';
    formData.status = 'NORMAL';
    formData.workCenterId = 0;
    formData.location = '';
    formData.remark = '';
  }
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (isEdit.value && currentId.value) {
        await updateMachine(currentId.value, formData);
...
const handleDelete = async (id: number) => {
  try {
    await ElMessageBox.confirm('确认删除该机台设备？', '提示', {
...
const openDetail = (row: Machine) => {
  currentRow.value = row;
  detailTab.value = 'base';
  detailVisible.value = true;
};

const openBizDialog = async (row: Machine, action: ResourceBizAction) => {
  try {
    let target = row;
    if (!target.id) {
      const res: any = await ensureLocalMachine(target);
      if (!res.success || !res.data?.id) {
        ElMessage.error(res.message || '缺少本地设备ID，无法执行业务操作');
        return;
      }
    ...
    ElMessage.error(error?.message || '准备本地设备档案失败');
  }
};
...
const handleExport = async () => {
  try {
    const blob = await exportMachine();
    downloadBlob(blob, '机台导出.xlsx');
  } catch {
...
<template>
  <div class="p-5">
    <div class="mb-4 flex items-center justify-between">
      <h1 class="text-lg font-semibold">机台设备</h1>
      <div class="flex gap-2">
        <el-button :icon="'Download'" @click="handleExport">导出</el-button>
        <el-button type="primary" @click="openDialog()" :icon="'Plus'">新建设备</el-button>
      </div>
    </div>
      ...
      <el-table :data="tableData" v-loading="loading" stripe border style="width: 100%" row-key="id">
        ...
        <el-table-column prop="status" label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="row.statusTagType || (row.status === 'NORMAL' ? 'success' : row.status === 'MAINTENANCE' ? 'warning' : row.status === 'FAULT' ? 'danger' : 'info')" size="small">
        ...
        <el-table-column label="业务操作" width="260" fixed="right">
          <template #default="{ row }">
            <ResourceBizActions resource-type="MACHINE" :row="row" @action="(action) => openBizDialog(row, action)" />
          </template>
        </el-table-column>
    ...
    <el-drawer v-model="detailVisible" title="机台详情" size="720px">
      <el-tabs v-model="detailTab">
        <el-tab-pane label="基本信息" name="base">
          <el-descriptions v-if="currentRow" :column="2" border>
            <el-descriptions-item label="设备编码">{{ currentRow.code }}</el-descriptions-item>
            <el-descriptions-item label="设备名称">{{ currentRow.name }}</el-descriptions-item>
            <el-descriptions-item label="规格型号">{{ currentRow.model || '-' }}</el-descriptions-item>
            <el-descriptions-item label="状态">{{ currentRow.statusText || currentRow.status || '-' }}</el-descriptions-item>
            <el-descriptions-item label="工作中心">{{ currentRow.workCenterId || '-' }}</el-descriptions-item>
